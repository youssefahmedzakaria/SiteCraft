  ...
  return (
    <div className={cn("w-full flex-shrink-0", bgColor)}>
      <div className={cn(" mx-auto px-16 py-8 md:py-16", textColor, fontFamily)}>
        {showTitle && (
          <h2 className={cn("text-4xl md:text-4xl font-bold text-center pb-4 mb-6", titleColor, titleFontSize, titleFont)}>{title}</h2>
        )}
        ...
        <div className={cn("grid grid-cols-1 md:grid-cols-3 w-full", gap)}>
          ...
          {featuredProduct && (
            <div className="md:col-span-2 md:row-span-2 group w-full">
              <div className={cn("relative h-96 md:h-full bg-slate-100 overflow-hidden w-full", borderRadius)}>
                <Image
                  src={featuredProduct.media?.mainMedia?.image?.url || "/placeholder.svg?height=600&width=800"}
                  alt={featuredProduct.name}
                  fill
                  sizes="(max-width: 768px) 100vw, 66vw"
                  className={cn("object-cover", hoverEffect && "transition-transform duration-500 group-hover:scale-105")}
                />
                <div className={cn("absolute inset-0 flex items-end bg-gradient-to-t from-black/60 to-transparent")}> 
                  <div className="p-6">
                    <h3 className="text-white text-2xl font-bold">{featuredProduct.name}</h3>
              ...
              <div className={cn("relative h-64 bg-slate-100 overflow-hidden w-full", borderRadius)}>
                ...
                <div className={cn("absolute inset-0 flex items-end bg-gradient-to-t from-black/60 to-transparent")}> 
                  <div className="p-4">
                    ...
                    {showCta && <p className="text-white/80 text-sm mt-1">{ctaText}</p>}
                  </div>
                </div>
              </div>
            </div>
