...
interface ProductLayoutItemsProps {
  selectedLayout: number;
  onLayoutSelect: (layoutId: number) => void;
}

export function ProductLayoutItems({
  selectedLayout,
  onLayoutSelect,
}: ProductLayoutItemsProps) {
  return (
    <div>
      <h3 className="font-medium mb-2">Product Layout</h3>
        ...
        {[1, 2, 3].map((layoutId) => (
          ...
          >
            ...
            <span className="text-xs font-semibold">
              {layoutId === 1 && "Grid"}
              {layoutId === 2 && "Featured Grid"}
              {layoutId === 3 && "Horizontal Scroll"}
            </span>
          </button>
        ))}
      </div>
