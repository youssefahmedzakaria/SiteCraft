...
export const useCategoryManagement = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.log('🔐 User authenticated for categories:', { userId: user.userId, storeId: user.storeId, role: user.role });
  ...
  const editCategory = async (categoryId: number, categoryData: CategoryCreateDTO, image?: File) => {
    try {
      setError(null);
      const updatedCategory = await updateCategory(categoryId, categoryData, image);
  ...
  const assignProductsToCategory = async (categoryId: number, productIds: number[]) => {
    try {
      setError(null);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to assign products to category';
      setError(errorMessage);
      throw err;
    }
  };
