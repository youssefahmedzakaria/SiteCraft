...
public class CartServiceIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        // Reset database sequences to avoid ID conflicts
        // Get the maximum ID from each table and set sequence to max + 1
        Long maxCustomerId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM customer", Long.class);
        Long maxProductId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM product", Long.class);
        Long maxVariantId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM productvariants", Long.class);
        Long maxCartId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM shoppingcart", Long.class);
        Long maxCartProductId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM cartproduct", Long.class);
        
        jdbcTemplate.execute("SELECT setval('customer_id_seq', " + (maxCustomerId + 1) + ", false)");
        jdbcTemplate.execute("SELECT setval('product_id_seq', " + (maxProductId + 1) + ", false)");
        jdbcTemplate.execute("SELECT setval('productvariants_id_seq', " + (maxVariantId + 1) + ", false)");
        jdbcTemplate.execute("SELECT setval('shoppingcart_id_seq', " + (maxCartId + 1) + ", false)");
        jdbcTemplate.execute("SELECT setval('cartproduct_id_seq', " + (maxCartProductId + 1) + ", false)");
